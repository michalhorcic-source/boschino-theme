...
document.addEventListener('click', (e) => {
  const btn = e.target.closest('.boschino-cta__btn');
  if (!btn) return;

  const cta = btn.closest('[data-boschino-cta]');
  if (!cta) return;

  // najdi viditelný input quantity v té stejné kartě
  const qtyInput =
    cta.closest('.card__content')?.querySelector('.quantity__input') ||
    cta.closest('.card')?.querySelector('.quantity__input') ||
    cta.closest('.card__content')?.querySelector('.boschino-quantity input[name="quantity"]') ||
    cta.closest('.card')?.querySelector('.boschino-quantity input[name="quantity"]');

  const hiddenQty = cta.querySelector('[data-boschino-qty-hidden]');
  if (!hiddenQty) return;
...
// Boschino qty controls (collection cards)
// Delegated handler = works with dynamic content
document.addEventListener('click', function (e) {
  const plus = e.target.closest('[data-qty-plus]');
  const minus = e.target.closest('[data-qty-minus]');
  if (!plus && !minus) return;

  const wrap = e.target.closest('[data-boschino-qty]');
  if (!wrap) return;
  ...
  // trigger events so any other scripts react
